...
def sync():
    # oci.config.from_file falls back to OCI_CONFIG_FILE env var if default location does not exist.
    # we want to give the env var precedence, so we need to specify it in the call
    oci_config = oci.config.from_file(os.getenv("OCI_CONFIG_FILE", oci.config.DEFAULT_LOCATION))

    service_endpoint = os.getenv("OCI_STREAM_SERVICE_ENDPOINT")
    stream_id = os.getenv("OCI_STREAM_ID")

    stream_client = oci.streaming.StreamClient(oci_config, service_endpoint=service_endpoint)
    cursor = get_cursor_by_group(stream_client, stream_id, "example-group", "example-instance-1")
    ...
    while True:
        ...
        for message in response.data:
            ...
            if event_type == "com.oraclecloud.computeapi.updateinstance":
              ...
              logging.info("Got {} event for resourceId {}".format(event_type, resource_id))
